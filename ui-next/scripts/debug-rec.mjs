...
try {
  ...
  // --record: thu frame screencast ra thư mục, đánh số tăng dần
  const recDir = (() => {
  ...
  if (recDir) {
    ...
    c.on("Page.screencastFrame", (p) => {
      writeFileSync(path.join(recDir, String(recN++).padStart(5, "0") + ".jpg"), Buffer.from(p.data, "base64"));
  ...
  // ---------- chạy step ----------
  if (recDir) await c.send("Page.startScreencast", { format: "jpeg", quality: 70, maxWidth: 1280, maxHeight: 800, everyNthFrame: 1 });
  ...
  if (recDir) {
    ...
    console.log(`[record] ${recN} frame -> ${recDir}`);
  }
